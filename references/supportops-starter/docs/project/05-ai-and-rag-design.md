...
Compare the classical baseline and LLM router on the same held-out test messages and label taxonomy. Report macro-F1, per-class precision/recall/F1, confusion matrix, sample counts, model call cost, and latency. Scikit-learn defines these metrics and supports their computation. [S6]
