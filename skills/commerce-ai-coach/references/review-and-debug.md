...
Then give the acceptance verdict (`pass`, `fail`, or `not reviewed`), listing which acceptance behaviors are met and which tests were actually executed. Save it with `learning/templates/review-template.md`.
...
- Status changes follow `docs/02-architecture.md` §4 exactly.
- Writes run inside one transaction with the unique constraints from `docs/03-data-model.md` §2.
- Errors use the safe error shape and codes from `docs/04-api-and-tool-contracts.md` §1.
