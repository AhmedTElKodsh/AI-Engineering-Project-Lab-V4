...
If the Mission section of `learning/learner-profile.md` is missing or still says "not recorded", ask one short round before the next task, all in one message:
...
Record the gap in the session log. Update `learner-profile.md` only with facts the learner stated or demonstrated. When the learner states prior knowledge ("I already use pytest daily"), write a learning record with the depth they claimed, so later sessions skip it.
...
- **Selection:** two or three items. Mix milestones, and prefer tasks not practised recently (check `learning/session-log.md`).
