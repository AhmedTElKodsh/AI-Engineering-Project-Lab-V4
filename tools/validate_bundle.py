...
Usage (from the V4 folder):
    python tools/validate_bundle.py            # run every check, exit 1 on any error
    python tools/validate_bundle.py --sync     # first refresh the two derived copies, then check

Derived copies (never edit them by hand):
  * projects/commerce-support-copilot/.github/skills/commerce-ai-coach/
      = skills/commerce-ai-coach/ without assets/
  * skills/commerce-ai-coach/assets/project-template/
      = projects/commerce-support-copilot/ without .github/skills/
...
PROJECT = ROOT / "projects" / "commerce-support-copilot"
SKILL = ROOT / "skills" / "commerce-ai-coach"
...
GITHUB_SOURCES = ["copilot-instructions.md", "pull-request-template.md",
...
def parse_backlog_md() -> dict[str, dict]:
    md = (PROJECT / "docs" / "09-backlog.md").read_text(encoding="utf-8")
...
def check_requirements() -> list[str]:
    prd = (PROJECT / "docs" / "01-prd.md").read_text(encoding="utf-8")
...
def check_seeds() -> list[str]:
    ...
    arch = (PROJECT / "docs" / "02-architecture.md").read_text(encoding="utf-8")
    case_states = set(re.findall(r"`([a-z_]+)`", arch.split("### Case workflow states")[1].split("|")[0]))
    evaluation = (PROJECT / "docs" / "06-evaluation-and-testing.md").read_text(encoding="utf-8")
...
def check_skill(folder: Path, standalone: bool) -> list[str]:
    ...
    if not (folder / "SKILL.md").is_file():
        hint = "" if standalone else " (run: python tools/validate_bundle.py --sync)"
...
def check_protocol_copy() -> list[str]:
    a = PROJECT / "learning" / "progress-protocol.md"
    b = SKILL / "references" / "progress-protocol.md"
    return [] if filecmp.cmp(a, b, shallow=False) else ["references/progress-protocol.md differs from learning/progress-protocol.md"]
...
def check_installed_helper() -> list[str]:
    if not (INSTALLED / "scripts" / "coach.py").is_file():
        return ["installed skill is missing (run: python tools/validate_bundle.py --sync)"]
